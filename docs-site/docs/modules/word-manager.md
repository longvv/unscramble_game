---
id: word-manager
title: Word Manager Module
sidebar_position: 5
---

# Word Manager Module

The Word Manager Module handles word list management and image association.

## Purpose

This module provides functionality for:
- Managing the list of words used in the game
- Associating images with words
- Adding and removing words from the word list
- Handling image uploads for words

## Implementation

The Word Manager uses the Module Pattern:
...
sequenceDiagram
    ...
    User->>WordManager: Enter new word
    User->>WordManager: Upload image
    WordManager->>WordManager: Process image file
    WordManager->>WordManager: Add word to list
    WordManager->>WordManager: Associate image with word
    WordManager->>UIFactory: Create word item
    WordManager->>StorageService: Save word data
    StorageService->>localStorage: Store data
    WordManager->>User: Update UI with new word
    
    User->>WordManager: Remove word
    WordManager->>WordManager: Remove from word list
    WordManager->>WordManager: Remove image association
    WordManager->>StorageService: Save updated data
    StorageService->>localStorage: Update stored data
    WordManager->>User: Update UI without word
```

## Key Features

- **Image Upload**: Handles file selection, drag-and-drop, and preview
- **Word Management**: Add, remove, and store words
- **Data Persistence**: Saves words and images to localStorage
- **Validation**: Ensures words are unique and valid
- **Programmatic API**: Methods for modifying the word list programmatically

## Public Methods

| Method | Description |
|--------|-------------|
| `init(elements)` | Initializes the word manager with DOM elements |
| `getWords()` | Returns a copy of all words |
| `getWordImage(word)` | Gets the image URL for a specific word |
| `addWord(word, imageUrl)` | Adds a word programmatically |
| `removeWord(word)` | Removes a word programmatically |

## Image Upload Process

1. User selects or drags an image file
2. File is validated to ensure it's an image
3. File is read as a data URL using FileReader
4. Image preview is displayed
5. When the word is added, the data URL is stored in the word images object
6. Image data is saved to localStorage for persistence

This module provides a comprehensive system for managing the game's word list, giving users the ability to customize their learning experience.
